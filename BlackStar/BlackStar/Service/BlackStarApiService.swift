...
class BlackStarApiService {
    ...
    static func loadProducts(for category: String, completion: @escaping (Result<DecodedArray<Product>, AFError>) -> Void) {

        AF.request("https://blackstarshop.ru/index.php?route=api/v1/products&cat_id=\(category)")
            .validate()
            .responseDecodable(of: DecodedArray<Product>.self) { response in
                DispatchQueue.main.async {
                    completion(response.result)
                }
        }
