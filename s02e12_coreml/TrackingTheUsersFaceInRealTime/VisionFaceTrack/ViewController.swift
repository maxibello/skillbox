...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    private func animateRocketTo(frame: CGRect) {
        let rocket = UIImageView(image: UIImage()) //картинка ракеты
        rocket.frame = CGRect(origin: CGPoint.zero, size: rocket.image!.size) //в примере было rocket.image!.size / 5
        rocket.center = CGPoint(x: view.frame.size.width / 2, y: view.frame.size.height - rocket.image!.size.height / 8)
        let boom = UIImageView(image: UIImage()) //картинка взрыва
        boom.frame = CGRect(origin: CGPoint.zero, size: boom.image!.size) //в примере было boom.image!.size / 4
